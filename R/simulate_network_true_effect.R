#' Compute true causal effects under network interference
#'
#' This function computes true causal estimands under a network interference
#' setting using the output from \code{simulate_network_outcome()}.
#'
#' The input must contain a list of individual potential outcome matrices
#' together with the corresponding node-level network data. For each
...
#' }
#'
#' @param sim_out A list returned by \code{simulate_network_outcome()}.
#' It must contain:
#' \describe{
...
#'   structure, including \code{subject_id} and \code{interference_set_size}}
#' }
...
#'
#' @return A list with class \code{"network_true_effect"} containing:
#' \describe{
...
#' simulate_network_true_effect(sim_out)
#'
#' @export
simulate_network_true_effect <- function(sim_out,
                                         allocations = c(0.25, 0.5, 0.75)) {

  if (!is.list(sim_out) || is.null(sim_out$PO) || is.null(sim_out$dat.network)) {
    stop("sim_out must be a list containing 'PO' and 'dat.network'.")
  }

  PO <- sim_out$PO
  dat.network <- sim_out$dat.network
